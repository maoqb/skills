...
3. **标签要短、不用括号；角色描述放小灰副标题**：**逐个检查每个方框**——框里的文字尽量短，
   不要塞进整句话，也**不要用括号写补充说明**（如 `组件名（某某层）`、`文件名（什么什么变量）`）。
   主名一行；若要补一句职责/出处/格式，用 `BlockDiagram.block(label, ..., subtitle="…")` 的
   `subtitle=` 参数——它会渲染成**第二行的小灰字副标题**（9px + 灰色 `#666`，自动不带括号），
   在视觉上明显次于主名。真实标识符（`PRODUCT_RELEASE_CONFIG_MAPS`、
   `inheritance_graph-<product>.dot` 等）本身较长是允许的，但**散文式的描述要么删、要么压成副标题**。
   一旦副标题里出现「A + B + C」这种并列，回到第 2 条改用框中框。
4. **箭头可以是折线**：`connect()` 支持 `waypoints=[(x,y), ...]`，让连线绕开其他方框、
   避免多条箭头挤在一起重叠。多条折线如果同向绕路，分配不同的 x/y 通道（例如分别走
   `x=30` 和 `x=100`），不要共用同一条通道。
5. 上述四条的具体写法和示例见 `references/block.md`。流程图/时序图的构建器目前没有这些
   API，但同样的“不溢出、不重叠、标签短”原则适用——必要时手动调整标签长度或换行。
