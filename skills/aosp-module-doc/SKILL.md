...
- **整体架构 → 框图 (block)：** 方框与子模块小节 1:1 对应（见第 3 节）。标签要有意义；这张图就是
  提纲。遵守 `drawio-diagrams` 技能里的质量约束：文字不超出方框（超长标签显式传 `w=`）；
  一个方框若包含多个并列子项（如某目录下的多种文件类型），用 `child_block()` 画框中框；
  连线密集时用 `connect(..., waypoints=[...])` 画折线避免箭头重叠。生成后用
  `rsvg-convert` 渲染成图片自查一遍。
